...
import { faPills, faPlus, faEdit } from "@fortawesome/free-solid-svg-icons";
import type { LaboratoryResponse } from "./hooks/useLaboratories";
import type { MedicineRequest, MedicineResponse } from "./hooks/useMedicines";

interface Props {
    medicine: MedicineResponse;
    laboratories: LaboratoryResponse[];
    onClose: () => void;
    onUpdate: (id: string, request: MedicineRequest) => Promise<boolean>;
    isSubmitting: boolean;
    error: string | null;
}

export default function EditMedicineModal({ medicine, laboratories, onClose, onUpdate, isSubmitting, error }: Props) {
    const [laboratoryId, setLaboratoryId] = useState(medicine.laboratoryId);
    const [brandName, setBrandName] = useState(medicine.brandName);
    const [genericName, setGenericName] = useState(medicine.genericName || "");
    const [atcCode, setAtcCode] = useState(medicine.atcCode);
    const [useFrom, setUseFrom] = useState(medicine.useFrom);
    ...
    const [compositionList, setCompositionList] = useState<{ key: string; value: string }[]>(
        medicine.composition ? Object.entries(medicine.composition).map(([key, value]) => ({ key, value: String(value) })) : []
    );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const success = await onUpdate(medicine.id, request);
        if (success) onClose();
    };
            ...
            <div className="bg-background rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl border border-surface-alt flex flex-col">
                <div className="px-6 py-4 border-b border-surface-alt flex justify-between items-center bg-surface sticky top-0">
                    <div className="flex items-center gap-3 text-primary">
                        ...
                        <h2 className="text-lg font-semibold text-text">Editar Medicamento</h2>
                    </div>
