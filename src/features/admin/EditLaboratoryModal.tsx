...
import { faFlask, faEdit } from "@fortawesome/free-solid-svg-icons";
...
export default function EditLaboratoryModal({ laboratory, onClose, onUpdate, isSubmitting, error }: Props) {
    const [name, setName] = useState(laboratory.name);

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        const success = await onUpdate(laboratory.id, name);
        if (success) onClose();
    };
            ...
            <div className="bg-background rounded-2xl w-full max-w-md overflow-hidden shadow-2xl border border-surface-alt flex flex-col">
                <div className="px-6 py-4 border-b border-surface-alt flex justify-between items-center bg-surface">
                    <div className="flex items-center gap-3 text-primary">
                        ...
                        <h2 className="text-lg font-semibold text-text">Editar Laboratorio</h2>
                    </div>
