...
import { faTrash, faExclamationTriangle, faFlask } from "@fortawesome/free-solid-svg-icons";
...
export default function DeleteLaboratoryModal({ laboratories, onClose, onDelete, isSubmitting, error }: Props) {
    const [selectedId, setSelectedId] = useState("");

    const handleDelete = async () => {
        if (!selectedId) return;
        if (!confirm("¿Está seguro de eliminar este laboratorio permanentemente?")) return;
            ...
            <div className="bg-background rounded-2xl w-full max-w-md overflow-hidden shadow-2xl border border-surface-alt flex flex-col">
                <div className="px-6 py-4 border-b border-surface-alt flex justify-between items-center bg-surface">
                    <div className="flex items-center gap-3 text-red-500">
                        ...
                        <h2 className="text-lg font-semibold text-text">Eliminar Laboratorio</h2>
                    </div>
                ...
                <div className="p-6 flex flex-col gap-4">
                    <div className="bg-orange-500/10 border border-orange-500/20 p-3 rounded-lg flex gap-3 text-orange-600 text-sm mb-2">
                        ...
                        <p>Asegúrese de que el laboratorio no esté asociado a ningún medicamento antes de eliminarlo.</p>
                    </div>

                    {error && <div className="p-3 bg-red-500/10 text-red-600 rounded-lg text-sm">{error}</div>}

                    <div>
                        <label className="block text-xs font-bold uppercase tracking-wider mb-1 text-text/70">Seleccione el laboratorio</label>
                        <select
                            value={selectedId}
